...
An RPML file is an XML document. The root element is `<page-el>`. No HTML wrapper, no doctype required. Import the renderer once:
...
```xml
<page-el title="..." route="/route" description="Snapshot shows [representative state]">
  ...
  <annotation-el id="1" label="Region Name">
    Spec prose.
    <enum-el>
      <enum-item label="State A" description="Trigger/condition."><!-- rp-* --></enum-item>
    </enum-el>
    <annotation-el label="Sub-region">Nested spec.</annotation-el>
  </annotation-el>
  <!-- one <annotation-el id="N"> per data-pin="N" -->
</page-el>
```
...
- `page-el` — root; title, route, description.
- `main-view` — scaled snapshot frame; `device`, `scale`, optional `width`/`height`.
- `viewport-el` — snapshot viewport; same `device` as main-view.
- `annotation-el` — specification block; top-level has `id` matching a pin, nested has no `id`.
- `enum-el` — horizontal container for mutually exclusive states.
- `enum-item` — one state card; `label` required, `description` optional.

**Primitive layer** — static UI building blocks used inside `main-view` and inside annotation `enum-item` bodies. ~109 elements across layout, controls, navigation, data display, feedback, enterprise, iOS, macOS, and agent families.

## Pin system

- Add `data-pin="N"` to any element inside `<main-view>`. Pins number from 1 with no gaps.
- Every `data-pin="N"` requires a matching `<annotation-el id="N">` at the top level of `page-el`.
...
|-------|-------------------|
| L1 | Page region (annotation-el with id) |
| L2 | Element or concern inside the region (nested annotation-el) |
| L3 | State family — mutually exclusive states (enum-el) |
...
Use `<enum-el>` for: state families (loaded/loading/empty/error), permission variants, validation branches, overlay results (open/closed, success/failure), and any conditional branch in code. Each `enum-item` gets an auto-numbered black square badge. Combinatorial states (permission × state) must be enumerated as products, not as separate flat lists.

## Overlay pattern

`modal-el`, `drawer-el`, `dropdown-el`, `popover-el`, `tooltip-el`, `toast-el` are **never placed in the main snapshot**. Pin the trigger element; render the overlay inside the trigger's annotation (usually inside `<enum-el>`).
