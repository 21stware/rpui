/** Minimal, dependency-free Markdown → HTML converter for the dev app docs view.
 *  Ported from the former tools/site/markdown.ts (the site generator is gone).
...
export function markdown(src) {
  ...
  while (i < lines.length) {
    ...
    while (
      i < lines.length &&
      lines[i].trim() &&
      !/^(#{1,6}\s|```|\||>\s?|\s*[-*+]\s|\s*\d+\.\s|(-{3,}|\*{3,})$)/.test(lines[i])
    ) {
      buf.push(lines[i]);
      i++;
    }
