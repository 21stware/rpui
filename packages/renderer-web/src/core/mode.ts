...
[data-rp-mode="edit"] .annotation-el-pane,[data-rp-mode="pick"] .annotation-el-pane{overflow:visible!important;max-height:none!important;height:auto!important}
[data-rp-mode="edit"] .rp-main-shell,[data-rp-mode="pick"] .rp-main-shell{height:auto!important;overflow:visible!important}
[data-rp-mode="edit"] .rp-main-stage-clip,[data-rp-mode="pick"] .rp-main-stage-clip{height:auto!important}
...
const INTERNAL = new Set([
  'rp-main-shell','rp-main-stage','rp-main-stage-clip','rp-pin',
  'page-el-shell','page-el-main','page-el-body','page-el-header',
  'annotation-el-pane','annotation-el-pane-inner',
]);

function isPickable(el: Element): boolean {
  return el.tagName.includes('-') && !INTERNAL.has(el.className.split(' ')[0] ?? '');
}
...
function buildPickInfo(el: Element): PickInfo {
  ...
  for (let i = 0; i < el.attributes.length; i++) {
    ...
    if (a.name !== 'data-rp-hover' && a.name !== 'data-rp-selected') attrs[a.name] = a.value;
  }
...
export class ModeManager {
  ...
  constructor(host: HTMLElement, opts: ModeManagerOpts = {}) {
    ...
    this._out = (e: PointerEvent) => {
      if (e.relatedTarget && this.host.contains(e.relatedTarget as Node)) return;
      this._hovered?.removeAttribute('data-rp-hover');
      this._hovered = null;
    };
    this._click = (e: PointerEvent) => {
      ...
      if (target.hasAttribute('data-rp-selected')) {
        ...
        this.host.querySelectorAll('[data-rp-selected]')
  ...
  setTheme(theme: RpuiTheme) { applyTheme(this.host, theme); }
  ...
  setSelected(selectors: string[]) {
    ...
    for (const sel of selectors) {
      try { this.host.querySelectorAll(sel).forEach(el => el.setAttribute('data-rp-selected', '')); }
