...
describe("rewriteTags", () => {
  ...
  test("preserves attributes and text", () => {
    expect(rewriteTags('<button label="Go" size="lg">')).toBe(
      '<button-el label="Go" size="lg">',
    );
  });

  test("does not rewrite inside attribute values", () => {
    // a literal "<page" inside a value never starts with "<" at token position
    expect(rewriteTags('<annotation label="see <page below">')).toBe(
      '<annotation-el label="see <page below">',
    );
  });
  ...
  test("word-boundary: table vs table-row, tab vs tabs", () => {
    expect(rewriteTags("<table><table-row></table-row></table>")).toBe(
      "<table-el><table-row></table-row></table-el>",
    );
    expect(rewriteTags("<tabs><tab></tab></tabs>")).toBe(
      "<tabs-el><tab-el></tab-el></tabs-el>",
    );
  });

  test("navigator does not collide with ios-navbar", () => {
    expect(rewriteTags("<navigator></navigator>")).toBe(
      "<navbar-el></navbar-el>",
    );
    expect(rewriteTags("<ios-navbar></ios-navbar>")).toBe(
  ...
  test("doc-mode typography primitives map verbatim (compounds)", () => {
    ...
    expect(toComponentTag("doc-ordered-list")).toBe("doc-ordered-list");
    expect(toComponentTag("doc-unordered-list")).toBe("doc-unordered-list");
    expect(toComponentTag("doc-quote")).toBe("doc-quote");
    expect(toComponentTag("code-inline")).toBe("code-inline");
  });

  test("inline formatting primitives gain -el", () => {
    expect(toComponentTag("bold")).toBe("bold-el");
    expect(toComponentTag("italic")).toBe("italic-el");
  });

  test("native HTML in text passes through", () => {
    expect(rewriteTags("<annotation>see <b>bold</b> here</annotation>")).toBe(
      "<annotation-el>see <b>bold</b> here</annotation-el>",
    );
  });

  test("comments are not rewritten", () => {
    expect(rewriteTags("<!-- page view button -->")).toBe(
      "<!-- page view button -->",
    );
  });
});
...
describe("normalize (expand + rewrite)", () => {
  test("self-closing expanded then rewritten", () => {
    expect(normalize('<logo label="App"/>')).toBe(
      '<logo-el label="App"></logo-el>',
    );
  });
});
