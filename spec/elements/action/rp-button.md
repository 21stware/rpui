...
- 允许的父元素：`rp-button-group`、`rp-navbar`、`rp-panel`、任意 `rp-*` 容器
- 允许的子元素：无（自闭合）

## 示例

```html
<rp-button label="新建任务" variant="primary" icon="plus"></rp-button>
<rp-button label="删除" variant="danger" state="loading"></rp-button>
```
