...
```xml
<rp-page title="..." route="..." description="...">
  ...
  <rp-annotation id="1" label="导航栏">...</rp-annotation>
  <rp-annotation id="2" label="筛选栏">...</rp-annotation>
...
`<rp-viewport>` 是快照内容的直接容器，`device` 属性应与 `rp-main-view` 一致。

## 双栏布局

渲染器自动将文档拆成两栏：

- **左栏**：标题头 + 主快照（`rp-main-view`）
- **右栏**：顶级 `<rp-annotation>` 列表，独立滚动
