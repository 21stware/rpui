...
```xml
<page-el title="..." route="..." description="...">
  ...
  <annotation-el id="1" label="导航栏">...</annotation-el>
  <annotation-el id="2" label="筛选栏">...</annotation-el>
...
`<viewport-el>` 是快照内容的直接容器，`device` 属性应与 `main-view` 一致。

## 双栏布局

渲染器自动将文档拆成两栏：

- **左栏**：标题头 + 主快照（`main-view`）
- **右栏**：顶级 `<annotation-el>` 列表，独立滚动
