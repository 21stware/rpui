...
RPML 通过**注释文字 + permission-gate + enum-el** 组合描述权限差异，不通过条件渲染实现。

## permission-gate

```xml
<permission-gate role="admin" label="仅管理员可见">
  <button-el label="删除项目" variant="danger" />
</permission-gate>
```
...
当权限与状态组合时，用 `enum-el` 穷举乘积：

```xml
<annotation-el label="操作按钮区权限矩阵">
  <enum-el>
