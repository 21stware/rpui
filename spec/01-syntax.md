...
```html
<page title="页面标题" route="/route" description="说明"> ... </page>
```

## 标签命名

所有 RPML 元素使用简洁的裸标签名（kebab-case，小写），例如 `page`、`view`、`button`、`table`。单词标签直接使用名称本身；复合名称保留连字符，例如 `list-item`、`table-row`、`enum-item`。平台原语使用 `ios-*` 前缀。
